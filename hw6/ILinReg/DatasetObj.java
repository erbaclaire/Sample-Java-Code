public class DatasetObj implements Dataset {
    ...
    public Line bestFit() {
        ...
        if ((this.numObs()*xSquaredSum)-(xSum*xSum)==0) throw new IllegalArgumentException("vertical line or same points - can not calculate");
	...
	for(int i=0;i<this.numObs();i++) {
            ...
            ySum += this.obs[i].getY();
            xSquaredSum += this.obs[i].getX()*this.obs[i].getX();
	    ySquaredSum += this.obs[i].getY()*this.obs[i].getY();
        }
	double avgXY = xySum/this.numObs();
	double avgX = xSum/this.numObs();
	double avgY = ySum/this.numObs();
	double avgXSquared = xSquaredSum/this.numObs();
	double avgYSquared = ySquaredSum/this.numObs();
	if(Math.sqrt(avgXSquared-(avgX*avgX))*Math.sqrt(avgYSquared-(avgY*avgY))==0) throw new IllegalArgumentException("vertical line, horizontal line, or same points - can not calculate");
	return Math.pow((avgXY-(avgX*avgY))/(Math.sqrt(avgXSquared-(avgX*avgX))*Math.sqrt(avgYSquared-(avgY*avgY))),2);
    }
}
